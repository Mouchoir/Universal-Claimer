...
function scripted(opts: {
  ...
  return {
    probe: {
      url: () => opts.url(t),
      status: () => opts.status,
      challenged: async () => t < (opts.challengedUntil ?? 0),
      sleep: async (ms) => {
        t += ms;
      },
      now: () => t,
...
/** A bare page: URL scripted against the time the driver has asked to wait, one document response. */
...
describe("PlaywrightEpicDriver.checkSignIn", () => {
  it("reads Cloudflare's cf-mitigated header as a challenge, and reports blocked when it stays", async () => {
    const driver = new PlaywrightEpicDriver(
      fakePage({ url: () => ACCOUNT, status: 403, headers: { "cf-mitigated": "challenge" } }),
      FAST,
    );
  ...
  it("recognises a challenge by its page markup when the header is absent", async () => {
    const driver = new PlaywrightEpicDriver(fakePage({ url: () => ACCOUNT, markers: 1 }), FAST);
    expect((await driver.checkSignIn()).state).toBe("blocked");
  });

  it("waits out the login page's bounce", async () => {
    const driver = new PlaywrightEpicDriver(
      fakePage({ url: (waited) => (waited < 15 ? LOGIN : ACCOUNT) }),
      { ...FAST, bounceMs: 5_000 },
    );
  ...
  it("is signed out when the login page stays", async () => {
    const driver = new PlaywrightEpicDriver(fakePage({ url: () => LOGIN }), FAST);
    expect(await driver.checkSignIn()).toMatchObject({ state: "signed_out", path: "/id/login" });
  });
});
